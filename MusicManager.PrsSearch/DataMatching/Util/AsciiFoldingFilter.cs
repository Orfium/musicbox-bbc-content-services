...
namespace MusicManager.PrsSearch.DataMatching.Util
{
    public class AsciiFoldingFilter
    {
        ...
        public string FoldToAscii(string input)
        {
            Func<int, int> getNextSize = targetSize => (targetSize >> 3) + (targetSize < 9 ? 3 : 6) + targetSize;

            // Worst-case length required:
            int maxSizeNeeded = 4 * input.Length;
        ...
        public static string Fold(string input, bool toUpper)
        {
            var result = new AsciiFoldingFilter().FoldToAscii(input);
